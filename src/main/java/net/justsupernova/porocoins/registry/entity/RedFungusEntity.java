...
public class RedFungusEntity extends PathAwareEntity implements Angerable {
    ...
    public void tickMovement() {
        ...
        if (!this.world.isClient) {
            ...
            int randomMushroom = this.world.random.nextInt(22000);
            if (randomMushroom == 1997 && world.getBlockState(this.getBlockPos().down()).isOf(Blocks.MYCELIUM) && this.world.getBlockState(this.getBlockPos()).isAir()) {
                this.world.setBlockState(this.getBlockPos(), Blocks.RED_MUSHROOM.getDefaultState());
            }
        }
    }
    ...
    public boolean tryAttack(Entity target) {
        if (!super.tryAttack(target)) {
            ...
            if (!this.world.isClient && target instanceof LivingEntity && this.world.random.nextInt(160) > 140) {
                ((LivingEntity) target).addStatusEffect(new StatusEffectInstance(StatusEffect.byRawId(19), 80 + this.world.random.nextInt(160), 0, false, false), this);
            }
            return true;
        }
    }
